class Tile:
    def __init__(self, tile: list, tile_id: int = 0):
        self.tile = tile
        self.id = tile_id
...
def check(order: list, tile: Tile, edge_size: int) -> bool:
...
def recursion(order, visited, tiles, edge_size) -> list:
...
def search(target: list, pic: Tile, print_to_terminal: bool) -> int:
...
def search_pic(target: list, pic: Tile, print_to_terminal: bool) -> int:
    for r in reassemble:
        r(pic)
...
with open("dec20/input.txt") as f:
    inputs = [
        line
